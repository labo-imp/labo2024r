...
FEintra_manual_base <- function( pinputexps )
{
  ...
  param_local$meta$script <- "/src/wf-etapas/1301_FE_intrames_manual_v_norm_adicionales_001.r"
...
CN_canaritos_asesinos_base <- function(ratio=0.2, desvio=0.5)
{
  if( -1 == (param_local <- exp_init())$resultado ) return( 0 )# linea fija
...
TS_strategy_base9 <- function( pinputexps )
{
  ...
  param_local$final_train$training <- c(2202107, 202106, 
  ...
  param_local$train$training <- c(201910, 201911, 201912, 202008, 202009,202010, 202101, 202102, 202103)
  param_local$train$validation <- c(202001, 202011, 202104)
  param_local$train$testing <- c(202002, 202012, 202106)
...
wf_septiembre <- function( pnombrewf )
{
  ...
  fm <- FM_final_models_lightgbm_semillerio( c(ht, ts9), ranks=c(1), semillerio=5, repeticiones_exp=1 )
